# ブロックを破壊または置換などで失わせるもの。とくに広範囲または回避が難しいもの。

data modify storage den_1:1mob1com difficulty.cow set value "1st"
data modify storage den_1:1mob1com difficulty.creeper set value "2nd"
data modify storage den_1:1mob1com difficulty.pig set value "1st"
data modify storage den_1:1mob1com difficulty.ravager set value "1st"
data modify storage den_1:1mob1com difficulty.spider set value "1st"
data modify storage den_1:1mob1com difficulty.vindicator set value "2nd"
data modify storage den_1:1mob1com difficulty.wither set value "2nd"
data modify storage den_1:1mob1com difficulty.zoglin set value "1st"
data modify storage den_1:1mob1com difficulty.zombie_villager set value "1st"
data modify storage den_1:1mob1com difficulty.zombified_piglin set value "1st"
